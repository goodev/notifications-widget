...
public class AppearancePreferencesFragment extends NiLSPreferenceFragment implements Preference.OnPreferenceChangeListener, ActionBar.OnNavigationListener {
    ...
    private void updatePreviewNow(boolean reloadPrefs)
    {
        ...
        NotificationAdapter.ViewHolder holder = new NotificationAdapter.ViewHolder(mPreviewView);
        NotificationAdapter.applySettingsToView(getActivity(), holder, mPreviewView, mPreviewNotificationItem, 0, mTheme, true);
        if (reloadPrefs)
        {
            try
            {
